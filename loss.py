...
def criterion(outputs, image_refs, labels, dist_maps, meshes, device):
    ...
    """
    return (
        2e-3 * mse_loss(outputs["joints"], labels)
        + 2e-2 * aligned_joints_loss(labels, outputs["joints"])
        + 2e-3 * mse_loss(outputs["refined_joints"], labels)
        + 2e-2 * aligned_joints_loss(labels, outputs["refined_joints"])
        + 0.1 * l1_loss(outputs["refined_vertices"], meshes)
...
if __name__ == "__main__":
    ...
    l1_loss = nn.L1Loss()
    contour_loss = ContourLoss(device=device)
    inputs = torch.rand(3, 224)
    labels = torch.rand(3, 224)
    out = mse_loss(inputs, labels)
    print(out)
    # out = aligned_joints_loss(labels, inputs)
    # print(out)
    out = l1_loss(inputs, labels)
    print(out)
    inputs = torch.rand(3, 3, 224)
    dist_maps = torch.rand(3, 224)
    out = contour_loss(inputs, dist_maps)
    print(out)
