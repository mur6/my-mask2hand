    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            label_uvd = self.world2pixel(
                np.array(self.joints[index]), fx=self.camera_Ks[index][0][0], fy=self.camera_Ks[index][1][1]
            )

            mesh_uvd = self.world2pixel(
                np.array(self.vertices[index]), fx=self.camera_Ks[index][0][0], fy=self.camera_Ks[index][1][1]
            )
